...
import { BaoMoiXemTinCrawler2 } from '@crawler/impl/BaoMoiXemTinCrawler2';
import { CreateQuery } from 'mongoose';

export class BaoMoiTinMoiCrawler2 extends NewsCrawler {
    ...
    constructor(page: number, piority: number = 5) {
        super(BaoMoiTinMoiCrawler2.getBMTMUrl(page), piority);
        this.page = page;
    }
    ...
    async parseHtml(html: string): Promise<CreateQuery<News> | null> {
        ...
        items.forEach(
            (value: string) => this.manager?.addNewCrawler(new BaoMoiXemTinCrawler2(value, this.priority - 1))
        );
        ...
        if(items.length !== 0) {
            ...
            this.manager?.addNewCrawler(new BaoMoiTinMoiCrawler2(this.page++, this.priority - 1));
        }
